...
import com.weilango.system.service.ISysDictDataService;
...
public class SysDictDataController extends BaseController
{
     ...
     * 根据类型查询字典信息（多个）
     */
    @ApiOperation("根据类型查询字典信息（多个）")
    @GetMapping(value = "/types/{dictTypes}")
    public AjaxResult dictTypes(@PathVariable String[] dictTypes)
    {
        List<Map<String,List<SysDictData>>> data = dictTypeService.selectDictDataByTypes(dictTypes);
        if (StringUtils.isNull(data))
        {
            data = new ArrayList<Map<String,List<SysDictData>>>();
        }
        return AjaxResult.success(data);
    }
